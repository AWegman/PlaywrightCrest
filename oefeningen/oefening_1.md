...
2. Open de Chat-interface in je editor (waar de Playwright agents beschikbaar zijn).
...
2. Typ de volgende prompt:

"Open de website http://localhost:5173/ en genereer een simpele test die kijkt of de website online is."

3. De agent zal de browser op de achtergrond openen, de pagina inspecteren en een codevoorstel doen.
4. Klik op Insert of Copy om de test in een nieuw testbestand te plaatsen.
...
2. Pas de verwachting (de expect) aan naar iets dat niet klopt.

Voorbeeld: Verander de tekst die gezocht wordt naar "Deze tekst bestaat niet".

3. Sla het bestand op en draai de test via de Playwright Sidebar of de terminal.
4. Resultaat: De test faalt (rood).

Stap 4: De playwright-test-healer inschakelen
Nu roepen we de hulp in van de reparatie-agent.
...
3. De healer-agent analyseert waarom de test faalt en vergelijkt de code met de actuele staat van de website op localhost:5173.
4. De agent komt met een voorstel om de selector of tekst te herstellen.
5. Klik op Keep of Accept om de fix door te voeren.

Stap 5: Verificatie

Sla de wijzigingen op.
2. Draai de test nogmaals.
3. Als het goed is, kleurt de test nu weer groen.
